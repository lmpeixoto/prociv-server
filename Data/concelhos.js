const { json } = require('express');

fs = require['fs'];

const concelhos = [
    [
        ['code', '010100'],
        ['name', 'ÁGUEDA']
    ],
    [
        ['code', '010200'],
        ['name', 'ALBERGARIA-A-VELHA']
    ],
    [
        ['code', '010300'],
        ['name', 'ANADIA']
    ],
    [
        ['code', '010400'],
        ['name', 'AROUCA']
    ],
    [
        ['code', '010500'],
        ['name', 'AVEIRO']
    ],
    [
        ['code', '010600'],
        ['name', 'CASTELO DE PAIVA']
    ],
    [
        ['code', '010700'],
        ['name', 'ESPINHO']
    ],
    [
        ['code', '010800'],
        ['name', 'ESTARREJA']
    ],
    [
        ['code', '010900'],
        ['name', 'SANTA MARIA DA FEIRA']
    ],
    [
        ['code', '011000'],
        ['name', 'ÍLHAVO']
    ],
    [
        ['code', '011100'],
        ['name', 'MEALHADA']
    ],
    [
        ['code', '011200'],
        ['name', 'MURTOSA']
    ],
    [
        ['code', '011300'],
        ['name', 'OLIVEIRA DE AZEMÉIS']
    ],
    [
        ['code', '011400'],
        ['name', 'OLIVEIRA DO BAIRRO']
    ],
    [
        ['code', '011500'],
        ['name', 'OVAR']
    ],
    [
        ['code', '011600'],
        ['name', 'SÃO JOÃO DA MADEIRA']
    ],
    [
        ['code', '011700'],
        ['name', 'SEVER DO VOUGA']
    ],
    [
        ['code', '011800'],
        ['name', 'VAGOS']
    ],
    [
        ['code', '011900'],
        ['name', 'VALE DE CAMBRA']
    ],
    [
        ['code', '020100'],
        ['name', 'ALJUSTREL']
    ],
    [
        ['code', '020200'],
        ['name', 'ALMODÔVAR']
    ],
    [
        ['code', '020300'],
        ['name', 'ALVITO']
    ],
    [
        ['code', '020400'],
        ['name', 'BARRANCOS']
    ],
    [
        ['code', '020500'],
        ['name', 'BEJA']
    ],
    [
        ['code', '020600'],
        ['name', 'CASTRO VERDE']
    ],
    [
        ['code', '020700'],
        ['name', 'CUBA']
    ],
    [
        ['code', '020800'],
        ['name', 'FERREIRA DO ALENTEJO']
    ],
    [
        ['code', '020900'],
        ['name', 'MÉRTOLA']
    ],
    [
        ['code', '021000'],
        ['name', 'MOURA']
    ],
    [
        ['code', '021100'],
        ['name', 'ODEMIRA']
    ],
    [
        ['code', '021200'],
        ['name', 'OURIQUE']
    ],
    [
        ['code', '021300'],
        ['name', 'SERPA']
    ],
    [
        ['code', '021400'],
        ['name', 'VIDIGUEIRA']
    ],
    [
        ['code', '030100'],
        ['name', 'AMARES']
    ],
    [
        ['code', '030200'],
        ['name', 'BARCELOS']
    ],
    [
        ['code', '030300'],
        ['name', 'BRAGA']
    ],
    [
        ['code', '030400'],
        ['name', 'CABECEIRAS DE BASTO']
    ],
    [
        ['code', '030500'],
        ['name', 'CELORICO DE BASTO']
    ],
    [
        ['code', '030600'],
        ['name', 'ESPOSENDE']
    ],
    [
        ['code', '030700'],
        ['name', 'FAFE']
    ],
    [
        ['code', '030800'],
        ['name', 'GUIMARÃES']
    ],
    [
        ['code', '030900'],
        ['name', 'PÓVOA DE LANHOSO']
    ],
    [
        ['code', '031000'],
        ['name', 'TERRAS DE BOURO']
    ],
    [
        ['code', '031100'],
        ['name', 'VIEIRA DO MINHO']
    ],
    [
        ['code', '031200'],
        ['name', 'VILA NOVA DE FAMALICÃO']
    ],
    [
        ['code', '031300'],
        ['name', 'VILA VERDE']
    ],
    [
        ['code', '031400'],
        ['name', 'VIZELA']
    ],
    [
        ['code', '040100'],
        ['name', 'ALFÂNDEGA DA FÉ']
    ],
    [
        ['code', '040200'],
        ['name', 'BRAGANÇA']
    ],
    [
        ['code', '040300'],
        ['name', 'CARRAZEDA DE ANSIÃES']
    ],
    [
        ['code', '040400'],
        ['name', 'FREIXO DE ESPADA À CINTA']
    ],
    [
        ['code', '040500'],
        ['name', 'MACEDO DE CAVALEIROS']
    ],
    [
        ['code', '040600'],
        ['name', 'MIRANDA DO DOURO']
    ],
    [
        ['code', '040700'],
        ['name', 'MIRANDELA']
    ],
    [
        ['code', '040800'],
        ['name', 'MOGADOURO']
    ],
    [
        ['code', '040900'],
        ['name', 'TORRE DE MONCORVO']
    ],
    [
        ['code', '041000'],
        ['name', 'VILA FLOR']
    ],
    [
        ['code', '041100'],
        ['name', 'VIMIOSO']
    ],
    [
        ['code', '041200'],
        ['name', 'VINHAIS']
    ],
    [
        ['code', '050100'],
        ['name', 'BELMONTE']
    ],
    [
        ['code', '050200'],
        ['name', 'CASTELO BRANCO']
    ],
    [
        ['code', '050300'],
        ['name', 'COVILHÃ']
    ],
    [
        ['code', '050400'],
        ['name', 'FUNDÃO']
    ],
    [
        ['code', '050500'],
        ['name', 'IDANHA-A-NOVA']
    ],
    [
        ['code', '050600'],
        ['name', 'OLEIROS']
    ],
    [
        ['code', '050700'],
        ['name', 'PENAMACOR']
    ],
    [
        ['code', '050800'],
        ['name', 'PROENÇA-A-NOVA']
    ],
    [
        ['code', '050900'],
        ['name', 'SERTÃ']
    ],
    [
        ['code', '051000'],
        ['name', 'VILA DE REI']
    ],
    [
        ['code', '051100'],
        ['name', 'VILA VELHA DE RÓDÃO']
    ],
    [
        ['code', '060100'],
        ['name', 'ARGANIL']
    ],
    [
        ['code', '060200'],
        ['name', 'CANTANHEDE']
    ],
    [
        ['code', '060300'],
        ['name', 'COIMBRA']
    ],
    [
        ['code', '060400'],
        ['name', 'CONDEIXA-A-NOVA']
    ],
    [
        ['code', '060500'],
        ['name', 'FIGUEIRA DA FOZ']
    ],
    [
        ['code', '060600'],
        ['name', 'GÓIS']
    ],
    [
        ['code', '060700'],
        ['name', 'LOUSÃ']
    ],
    [
        ['code', '060800'],
        ['name', 'MIRA']
    ],
    [
        ['code', '060900'],
        ['name', 'MIRANDA DO CORVO']
    ],
    [
        ['code', '061000'],
        ['name', 'MONTEMOR-O-VELHO']
    ],
    [
        ['code', '061100'],
        ['name', 'OLIVEIRA DO HOSPITAL']
    ],
    [
        ['code', '061200'],
        ['name', 'PAMPILHOSA DA SERRA']
    ],
    [
        ['code', '061300'],
        ['name', 'PENACOVA']
    ],
    [
        ['code', '061400'],
        ['name', 'PENELA']
    ],
    [
        ['code', '061500'],
        ['name', 'SOURE']
    ],
    [
        ['code', '061600'],
        ['name', 'TÁBUA']
    ],
    [
        ['code', '061700'],
        ['name', 'VILA NOVA DE POIARES']
    ],
    [
        ['code', '070100'],
        ['name', 'ALANDROAL']
    ],
    [
        ['code', '070200'],
        ['name', 'ARRAIOLOS']
    ],
    [
        ['code', '070300'],
        ['name', 'BORBA']
    ],
    [
        ['code', '070400'],
        ['name', 'ESTREMOZ']
    ],
    [
        ['code', '070500'],
        ['name', 'ÉVORA']
    ],
    [
        ['code', '070600'],
        ['name', 'MONTEMOR-O-NOVO']
    ],
    [
        ['code', '070700'],
        ['name', 'MORA']
    ],
    [
        ['code', '070800'],
        ['name', 'MOURÃO']
    ],
    [
        ['code', '070900'],
        ['name', 'PORTEL']
    ],
    [
        ['code', '071000'],
        ['name', 'REDONDO']
    ],
    [
        ['code', '071100'],
        ['name', 'REGUENGOS DE MONSARAZ']
    ],
    [
        ['code', '071200'],
        ['name', 'VENDAS NOVAS']
    ],
    [
        ['code', '071300'],
        ['name', 'VIANA DO ALENTEJO']
    ],
    [
        ['code', '071400'],
        ['name', 'VILA VIÇOSA']
    ],
    [
        ['code', '080100'],
        ['name', 'ALBUFEIRA']
    ],
    [
        ['code', '080200'],
        ['name', 'ALCOUTIM']
    ],
    [
        ['code', '080300'],
        ['name', 'ALJEZUR']
    ],
    [
        ['code', '080400'],
        ['name', 'CASTRO MARIM']
    ],
    [
        ['code', '080500'],
        ['name', 'FARO']
    ],
    [
        ['code', '080600'],
        ['name', 'LAGOA']
    ],
    [
        ['code', '080700'],
        ['name', 'LAGOS']
    ],
    [
        ['code', '080800'],
        ['name', 'LOULÉ']
    ],
    [
        ['code', '080900'],
        ['name', 'MONCHIQUE']
    ],
    [
        ['code', '081000'],
        ['name', 'OLHÃO']
    ],
    [
        ['code', '081100'],
        ['name', 'PORTIMÃO']
    ],
    [
        ['code', '081200'],
        ['name', 'SÃO BRÁS DE ALPORTEL']
    ],
    [
        ['code', '081300'],
        ['name', 'SILVES']
    ],
    [
        ['code', '081400'],
        ['name', 'TAVIRA']
    ],
    [
        ['code', '081500'],
        ['name', 'VILA DO BISPO']
    ],
    [
        ['code', '081600'],
        ['name', 'VILA REAL DE SANTO ANTÓNIO']
    ],
    [
        ['code', '090100'],
        ['name', 'AGUIAR DA BEIRA']
    ],
    [
        ['code', '090200'],
        ['name', 'ALMEIDA']
    ],
    [
        ['code', '090300'],
        ['name', 'CELORICO DA BEIRA']
    ],
    [
        ['code', '090400'],
        ['name', 'FIGUEIRA DE CASTELO RODRIGO']
    ],
    [
        ['code', '090500'],
        ['name', 'FORNOS DE ALGODRES']
    ],
    [
        ['code', '090600'],
        ['name', 'GOUVEIA']
    ],
    [
        ['code', '090700'],
        ['name', 'GUARDA']
    ],
    [
        ['code', '090800'],
        ['name', 'MANTEIGAS']
    ],
    [
        ['code', '090900'],
        ['name', 'MÊDA']
    ],
    [
        ['code', '091000'],
        ['name', 'PINHEL']
    ],
    [
        ['code', '091100'],
        ['name', 'SABUGAL']
    ],
    [
        ['code', '091200'],
        ['name', 'SEIA']
    ],
    [
        ['code', '091300'],
        ['name', 'TRANCOSO']
    ],
    [
        ['code', '091400'],
        ['name', 'VILA NOVA DE FOZ CÔA']
    ],
    [
        ['code', '100100'],
        ['name', 'ALCOBAÇA']
    ],
    [
        ['code', '100200'],
        ['name', 'ALVAIÁZERE']
    ],
    [
        ['code', '100300'],
        ['name', 'ANSIÃO']
    ],
    [
        ['code', '100400'],
        ['name', 'BATALHA']
    ],
    [
        ['code', '100500'],
        ['name', 'BOMBARRAL']
    ],
    [
        ['code', '100600'],
        ['name', 'CALDAS DA RAINHA']
    ],
    [
        ['code', '100700'],
        ['name', 'CASTANHEIRA DE PÊRA']
    ],
    [
        ['code', '100800'],
        ['name', 'FIGUEIRÓ DOS VINHOS']
    ],
    [
        ['code', '100900'],
        ['name', 'LEIRIA']
    ],
    [
        ['code', '101000'],
        ['name', 'MARINHA GRANDE']
    ],
    [
        ['code', '101100'],
        ['name', 'NAZARÉ']
    ],
    [
        ['code', '101200'],
        ['name', 'ÓBIDOS']
    ],
    [
        ['code', '101300'],
        ['name', 'PEDRÓGÃO GRANDE']
    ],
    [
        ['code', '101400'],
        ['name', 'PENICHE']
    ],
    [
        ['code', '101500'],
        ['name', 'POMBAL']
    ],
    [
        ['code', '101600'],
        ['name', 'PORTO DE MÓS']
    ],
    [
        ['code', '110100'],
        ['name', 'ALENQUER']
    ],
    [
        ['code', '110200'],
        ['name', 'ARRUDA DOS VINHOS']
    ],
    [
        ['code', '110300'],
        ['name', 'AZAMBUJA']
    ],
    [
        ['code', '110400'],
        ['name', 'CADAVAL']
    ],
    [
        ['code', '110500'],
        ['name', 'CASCAIS']
    ],
    [
        ['code', '110600'],
        ['name', 'LISBOA']
    ],
    [
        ['code', '110700'],
        ['name', 'LOURES']
    ],
    [
        ['code', '110800'],
        ['name', 'LOURINHÃ']
    ],
    [
        ['code', '110900'],
        ['name', 'MAFRA']
    ],
    [
        ['code', '111000'],
        ['name', 'OEIRAS']
    ],
    [
        ['code', '111100'],
        ['name', 'SINTRA']
    ],
    [
        ['code', '111200'],
        ['name', 'SOBRAL DE MONTE AGRAÇO']
    ],
    [
        ['code', '111300'],
        ['name', 'TORRES VEDRAS']
    ],
    [
        ['code', '111400'],
        ['name', 'VILA FRANCA DE XIRA']
    ],
    [
        ['code', '111500'],
        ['name', 'AMADORA']
    ],
    [
        ['code', '111600'],
        ['name', 'ODIVELAS']
    ],
    [
        ['code', '120100'],
        ['name', 'ALTER DO CHÃO']
    ],
    [
        ['code', '120200'],
        ['name', 'ARRONCHES']
    ],
    [
        ['code', '120300'],
        ['name', 'AVIS']
    ],
    [
        ['code', '120400'],
        ['name', 'CAMPO MAIOR']
    ],
    [
        ['code', '120500'],
        ['name', 'CASTELO DE VIDE']
    ],
    [
        ['code', '120600'],
        ['name', 'CRATO']
    ],
    [
        ['code', '120700'],
        ['name', 'ELVAS']
    ],
    [
        ['code', '120800'],
        ['name', 'FRONTEIRA']
    ],
    [
        ['code', '120900'],
        ['name', 'GAVIÃO']
    ],
    [
        ['code', '121000'],
        ['name', 'MARVÃO']
    ],
    [
        ['code', '121100'],
        ['name', 'MONFORTE']
    ],
    [
        ['code', '121200'],
        ['name', 'NISA']
    ],
    [
        ['code', '121300'],
        ['name', 'PONTE DE SOR']
    ],
    [
        ['code', '121400'],
        ['name', 'PORTALEGRE']
    ],
    [
        ['code', '121500'],
        ['name', 'SOUSEL']
    ],
    [
        ['code', '130100'],
        ['name', 'AMARANTE']
    ],
    [
        ['code', '130200'],
        ['name', 'BAIÃO']
    ],
    [
        ['code', '130300'],
        ['name', 'FELGUEIRAS']
    ],
    [
        ['code', '130400'],
        ['name', 'GONDOMAR']
    ],
    [
        ['code', '130500'],
        ['name', 'LOUSADA']
    ],
    [
        ['code', '130600'],
        ['name', 'MAIA']
    ],
    [
        ['code', '130700'],
        ['name', 'MARCO DE CANAVESES']
    ],
    [
        ['code', '130800'],
        ['name', 'MATOSINHOS']
    ],
    [
        ['code', '130900'],
        ['name', 'PAÇOS DE FERREIRA']
    ],
    [
        ['code', '131000'],
        ['name', 'PAREDES']
    ],
    [
        ['code', '131100'],
        ['name', 'PENAFIEL']
    ],
    [
        ['code', '131200'],
        ['name', 'PORTO']
    ],
    [
        ['code', '131300'],
        ['name', 'PÓVOA DE VARZIM']
    ],
    [
        ['code', '131400'],
        ['name', 'SANTO TIRSO']
    ],
    [
        ['code', '131500'],
        ['name', 'VALONGO']
    ],
    [
        ['code', '131600'],
        ['name', 'VILA DO CONDE']
    ],
    [
        ['code', '131700'],
        ['name', 'VILA NOVA DE GAIA']
    ],
    [
        ['code', '131800'],
        ['name', 'TROFA']
    ],
    [
        ['code', '140100'],
        ['name', 'ABRANTES']
    ],
    [
        ['code', '140200'],
        ['name', 'ALCANENA']
    ],
    [
        ['code', '140300'],
        ['name', 'ALMEIRIM']
    ],
    [
        ['code', '140400'],
        ['name', 'ALPIARÇA']
    ],
    [
        ['code', '140500'],
        ['name', 'BENAVENTE']
    ],
    [
        ['code', '140600'],
        ['name', 'CARTAXO']
    ],
    [
        ['code', '140700'],
        ['name', 'CHAMUSCA']
    ],
    [
        ['code', '140800'],
        ['name', 'CONSTÂNCIA']
    ],
    [
        ['code', '140900'],
        ['name', 'CORUCHE']
    ],
    [
        ['code', '141000'],
        ['name', 'ENTRONCAMENTO']
    ],
    [
        ['code', '141100'],
        ['name', 'FERREIRA DO ZÊZERE']
    ],
    [
        ['code', '141200'],
        ['name', 'GOLEGÃ']
    ],
    [
        ['code', '141300'],
        ['name', 'MAÇÃO']
    ],
    [
        ['code', '141400'],
        ['name', 'RIO MAIOR']
    ],
    [
        ['code', '141500'],
        ['name', 'SALVATERRA DE MAGOS']
    ],
    [
        ['code', '141600'],
        ['name', 'SANTARÉM']
    ],
    [
        ['code', '141700'],
        ['name', 'SARDOAL']
    ],
    [
        ['code', '141800'],
        ['name', 'TOMAR']
    ],
    [
        ['code', '141900'],
        ['name', 'TORRES NOVAS']
    ],
    [
        ['code', '142000'],
        ['name', 'VILA NOVA DA BARQUINHA']
    ],
    [
        ['code', '142100'],
        ['name', 'OURÉM']
    ],
    [
        ['code', '150100'],
        ['name', 'ALCÁCER DO SAL']
    ],
    [
        ['code', '150200'],
        ['name', 'ALCOCHETE']
    ],
    [
        ['code', '150300'],
        ['name', 'ALMADA']
    ],
    [
        ['code', '150400'],
        ['name', 'BARREIRO']
    ],
    [
        ['code', '150500'],
        ['name', 'GRÂNDOLA']
    ],
    [
        ['code', '150600'],
        ['name', 'MOITA']
    ],
    [
        ['code', '150700'],
        ['name', 'MONTIJO']
    ],
    [
        ['code', '150800'],
        ['name', 'PALMELA']
    ],
    [
        ['code', '150900'],
        ['name', 'SANTIAGO DO CACÉM']
    ],
    [
        ['code', '151000'],
        ['name', 'SEIXAL']
    ],
    [
        ['code', '151100'],
        ['name', 'SESIMBRA']
    ],
    [
        ['code', '151200'],
        ['name', 'SETÚBAL']
    ],
    [
        ['code', '151300'],
        ['name', 'SINES']
    ],
    [
        ['code', '160100'],
        ['name', 'ARCOS DE VALDEVEZ']
    ],
    [
        ['code', '160200'],
        ['name', 'CAMINHA']
    ],
    [
        ['code', '160300'],
        ['name', 'MELGAÇO']
    ],
    [
        ['code', '160400'],
        ['name', 'MONÇÃO']
    ],
    [
        ['code', '160500'],
        ['name', 'PAREDES DE COURA']
    ],
    [
        ['code', '160600'],
        ['name', 'PONTE DA BARCA']
    ],
    [
        ['code', '160700'],
        ['name', 'PONTE DE LIMA']
    ],
    [
        ['code', '160800'],
        ['name', 'VALENÇA']
    ],
    [
        ['code', '160900'],
        ['name', 'VIANA DO CASTELO']
    ],
    [
        ['code', '161000'],
        ['name', 'VILA NOVA DE CERVEIRA']
    ],
    [
        ['code', '170100'],
        ['name', 'ALIJÓ']
    ],
    [
        ['code', '170200'],
        ['name', 'BOTICAS']
    ],
    [
        ['code', '170300'],
        ['name', 'CHAVES']
    ],
    [
        ['code', '170400'],
        ['name', 'MESÃO FRIO']
    ],
    [
        ['code', '170500'],
        ['name', 'MONDIM DE BASTO']
    ],
    [
        ['code', '170600'],
        ['name', 'MONTALEGRE']
    ],
    [
        ['code', '170700'],
        ['name', 'MURÇA']
    ],
    [
        ['code', '170800'],
        ['name', 'PESO DA RÉGUA']
    ],
    [
        ['code', '170900'],
        ['name', 'RIBEIRA DE PENA']
    ],
    [
        ['code', '171000'],
        ['name', 'SABROSA']
    ],
    [
        ['code', '171100'],
        ['name', 'SANTA MARTA DE PENAGUIÃO']
    ],
    [
        ['code', '171200'],
        ['name', 'VALPAÇOS']
    ],
    [
        ['code', '171300'],
        ['name', 'VILA POUCA DE AGUIAR']
    ],
    [
        ['code', '171400'],
        ['name', 'VILA REAL']
    ],
    [
        ['code', '180100'],
        ['name', 'ARMAMAR']
    ],
    [
        ['code', '180200'],
        ['name', 'CARREGAL DO SAL']
    ],
    [
        ['code', '180300'],
        ['name', 'CASTRO DAIRE']
    ],
    [
        ['code', '180400'],
        ['name', 'CINFÃES']
    ],
    [
        ['code', '180500'],
        ['name', 'LAMEGO']
    ],
    [
        ['code', '180600'],
        ['name', 'MANGUALDE']
    ],
    [
        ['code', '180700'],
        ['name', 'MOIMENTA DA BEIRA']
    ],
    [
        ['code', '180800'],
        ['name', 'MORTÁGUA']
    ],
    [
        ['code', '180900'],
        ['name', 'NELAS']
    ],
    [
        ['code', '181000'],
        ['name', 'OLIVEIRA DE FRADES']
    ],
    [
        ['code', '181100'],
        ['name', 'PENALVA DO CASTELO']
    ],
    [
        ['code', '181200'],
        ['name', 'PENEDONO']
    ],
    [
        ['code', '181300'],
        ['name', 'RESENDE']
    ],
    [
        ['code', '181400'],
        ['name', 'SANTA COMBA DÃO']
    ],
    [
        ['code', '181500'],
        ['name', 'SÃO JOÃO DA PESQUEIRA']
    ],
    [
        ['code', '181600'],
        ['name', 'SÃO PEDRO DO SUL']
    ],
    [
        ['code', '181700'],
        ['name', 'SÁTÃO']
    ],
    [
        ['code', '181800'],
        ['name', 'SERNANCELHE']
    ],
    [
        ['code', '181900'],
        ['name', 'TABUAÇO']
    ],
    [
        ['code', '182000'],
        ['name', 'TAROUCA']
    ],
    [
        ['code', '182100'],
        ['name', 'TONDELA']
    ],
    [
        ['code', '182200'],
        ['name', 'VILA NOVA DE PAIVA']
    ],
    [
        ['code', '182300'],
        ['name', 'VISEU']
    ],
    [
        ['code', '182400'],
        ['name', 'VOUZELA']
    ],
    [
        ['code', '310100'],
        ['name', 'CALHETA']
    ],
    [
        ['code', '310200'],
        ['name', 'CÂMARA DE LOBOS']
    ],
    [
        ['code', '310300'],
        ['name', 'FUNCHAL']
    ],
    [
        ['code', '310400'],
        ['name', 'MACHICO']
    ],
    [
        ['code', '310500'],
        ['name', 'PONTA DO SOL']
    ],
    [
        ['code', '310600'],
        ['name', 'PORTO MONIZ']
    ],
    [
        ['code', '310700'],
        ['name', 'RIBEIRA BRAVA']
    ],
    [
        ['code', '310800'],
        ['name', 'SANTA CRUZ']
    ],
    [
        ['code', '310900'],
        ['name', 'SANTANA']
    ],
    [
        ['code', '311000'],
        ['name', 'SÃO VICENTE']
    ],
    [
        ['code', '320100'],
        ['name', 'PORTO SANTO']
    ],
    [
        ['code', '410100'],
        ['name', 'VILA DO PORTO']
    ],
    [
        ['code', '420100'],
        ['name', 'LAGOA']
    ],
    [
        ['code', '420200'],
        ['name', 'NORDESTE']
    ],
    [
        ['code', '420300'],
        ['name', 'PONTA DELGADA']
    ],
    [
        ['code', '420400'],
        ['name', 'POVOAÇÃO']
    ],
    [
        ['code', '420500'],
        ['name', 'RIBEIRA GRANDE']
    ],
    [
        ['code', '420600'],
        ['name', 'VILA FRANCA DO CAMPO']
    ],
    [
        ['code', '430100'],
        ['name', 'ANGRA DO HEROÍSMO']
    ],
    [
        ['code', '430200'],
        ['name', 'PRAIA DA VITÓRIA']
    ],
    [
        ['code', '440100'],
        ['name', 'SANTA CRUZ DA GRACIOSA']
    ],
    [
        ['code', '450100'],
        ['name', 'CALHETA DE SÃO JORGE']
    ],
    [
        ['code', '450200'],
        ['name', 'VELAS']
    ],
    [
        ['code', '460100'],
        ['name', 'LAJES DO PICO']
    ],
    [
        ['code', '460200'],
        ['name', 'MADALENA']
    ],
    [
        ['code', '460300'],
        ['name', 'SÃO ROQUE DO PICO']
    ],
    [
        ['code', '470100'],
        ['name', 'HORTA']
    ],
    [
        ['code', '480100'],
        ['name', 'LAJES DAS FLORES']
    ],
    [
        ['code', '480200'],
        ['name', 'SANTA CRUZ DAS FLORES']
    ],
    [
        ['code', '490100'],
        ['name', 'CORVO']
    ]
];

const distritos = [
    ['code', '010000', 'name', 'AVEIRO'],
    ['code', '020000', 'name', 'BEJA'],
    ['code', '030000', 'name', 'BRAGA'],
    ['code', '040000', 'name', 'BRAGANÇA'],
    ['code', '050000', 'name', 'CASTELO BRANCO'],
    ['code', '060000', 'name', 'COIMBRA'],
    ['code', '070000', 'name', 'ÉVORA'],
    ['code', '080000', 'name', 'FARO'],
    ['code', '090000', 'name', 'GUARDA'],
    ['code', '100000', 'name', 'LEIRIA'],
    ['code', '110000', 'name', 'LISBOA'],
    ['code', '120000', 'name', 'PORTALEGRE'],
    ['code', '130000', 'name', 'PORTO'],
    ['code', '140000', 'name', 'SANTARÉM'],
    ['code', '150000', 'name', 'SETÚBAL'],
    ['code', '160000', 'name', 'VIANA DO CASTELO'],
    ['code', '170000', 'name', 'VILA REAL'],
    ['code', '180000', 'name', 'VISEU'],
    ['code', '300000', 'name', 'MADEIRA'],
    ['code', '400000', 'name', 'AÇORES']
];

let jsonConcelhos = {};

function createJSONFile() {
    distritos.forEach((linha) => {
        let value = linha[1];
        value = value.substring(0, value.length - 4);
        let key = linha[3];
        jsonConcelhos[value] = key;
    });
    console.log(jsonConcelhos);
}

function writeToFile(content) {
    fs.writeFile('concelhos.json', JSON.parse(content), function (err) {
        if (err) throw err;
        console.log('Saved!');
    });
}

createJSONFile();
